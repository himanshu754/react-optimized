...
// ✅ UPDATED: Point to local proxy (Vercel rewrites)
const API_URL = '/api';
...
function App() {
  ...
  const analyzeImage = async () => {
    ...
    try {
      // ✅ UPDATED: Uses the proxy URL instead of direct link
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'} transition-colors duration-500`}>
      ...
      {/* Note: Ensure Chatbot.js is also using API_URL='/api' as we discussed before */}
